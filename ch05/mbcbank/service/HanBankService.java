...
public class HanBankService { 

// 서비스 계층은 CRUD 메서드 위주로 생성한다.
	// 필드
	// 생성자
	
	// 매서드 (부메뉴, 계좌생성, 입금, 출금, 삭제, 이체 등 ...)
	
	public void menu (Scanner inputInt, Scanner inputStr, AccountDTO[] hanaBank) {
		boolean subRun = true ;
		while (subRun) {
			System.out.println("-----------------------------");
			System.out.println("하나은행 계좌관리 메뉴 입니다.");
	 ...
	 // 계좌번호를 입력하고 입금액을 입력하면 계좌번호를 찾아서 계좌잔액을 증가 시킨다.
    	 ...
    	 AccountDTO inputAccount = findAccount(ano,hanaBank); // findAccount 메서드는 배열에서 객체를 찾아주고 객체를 리턴해준다.(반복코드 배제용)
	...
	private void createAccount(Scanner inputInt, Scanner inputStr, AccountDTO[] hanaBank ) {
	     ...
	     AccountDTO myAccount = new AccountDTO(); // 빈객체생성 
	     ...
	     for(int i = 0 ; i<hanaBank.length ; i ++) {
	    	if(hanaBank[i] == null) {
	    		...
	    		hanaBank[i] = myAccount ; // 배열에 입력한 객체가 삽입 (연결)
